...
class InferenceServer(abc.ABC):
    ...
    @abc.abstractmethod
    def _get_gateway_for_test(self, data_paths, file_share_dir=None):
        # Must return a version of the competition-specific gateway able to load data for unit tests.
        raise NotImplementedError

    def run_local_gateway(self, data_paths:Optional[Tuple[str]]=None, file_share_dir:str=None):
        ...
        self.server.start()
        try:
            self.gateway = self._get_gateway_for_test(data_paths, file_share_dir)
            self.gateway.run()
