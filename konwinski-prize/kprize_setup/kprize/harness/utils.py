...
from kprize.git_utils import get_input_repo_path_from_github_repo_name, \
    request_file_from_local_repo
...
def get_environment_yml_by_commit(repo: str, commit: str, env_name: str) -> str:
    input_repo_path = get_input_repo_path_from_github_repo_name(repo)
    # if local repo exists, use local file request instead of HTTP request
    make_request = request_file_from_local_repo if input_repo_path.exists() else requests.get
    for req_path in MAP_REPO_TO_ENV_YML_PATHS[repo]:
        reqs_url = os.path.join(SWE_BENCH_URL_RAW, repo, commit, req_path)
        reqs = make_request(reqs_url)
