...
from kprize.collection.configs.utils import get_repo_test_spec


DIFF_MODIFIED_FILE_REGEX = r"--- a/(.*)"

# THIS IS DUPLICATED FROM SWEBENCH TO PREVENT CIRCULAR DEPENDENCIES
...
class TestSpec:
    ...
    def base_image_key(self):
        return f"sweb.base.{self.arch}:latest".lower()
    ...
    def env_image_key(self):
        ...
        return f"sweb.env.{self.arch}.{val}:latest".lower()

    @property
    def instance_image_key(self):
        return f"sweb.eval.{self.arch}.{self.instance_id}:latest".lower()

    def get_instance_container_name(self, run_id=None):
        if not run_id:
            return f"sweb.eval.{self.instance_id}".lower()
        return f"sweb.eval.{self.instance_id}.{run_id}".lower()
...
def make_eval_script_list(instance, specs, env_name, repo_directory, base_commit, test_patch):
    ...
    test_command = " ".join(
        [
            get_repo_test_spec(instance["repo"], instance["version"])["test_cmd"],
...
def make_test_spec(instance: SWEbenchInstance) -> TestSpec:
    ...
    specs = get_repo_test_spec(repo, version)
